...
document.getElementById('backspace').onclick = () => {
    display.value == 'ERR' ? display.value = '' : display.value = display.value.substring(0, display.value.length-1);
}
...
document.getElementById('equals').onclick = () => {
    ...
    (tooLong(result)) ? display.value = result.toExponential(2) : display.value = result;
}

// adds a new character into the expression
function addToExpression(newVal) {
    display.value == 'ERR' ? display.value = newVal : display.value += newVal;
}
